...
Server.prototype.getResponse = function getResponse(req, cb) {
  ...
  if(certdb.includes("mysql://")) {
    async.map([serial], select, function(err, data) {
      logger.debug(`CERTDB contains mysql:// - looking for cert in remote database`)
      logger.trace(data);
      var certStatus = {
        type: null,
        value: null
      ...
      if (data[0] === '') {
        ...
        switch (status) {
          ...
          case 'Revoked': {
            ...
            response.certStatus.type = 'revoked';
            response.certStatus.value = { 
              revocationTime: Date.parse(data[0].RevokedAt),
              revocationReason: data[0].ReasonCode
            }
            break;
          };
          default: {
  ...
  function select(serial, cb) {
 
    var serialdecimal = parseInt(serial, 16);
    logger.info(`serialdecimal: ` + serialdecimal);

    var json = {}

    var database = mysql.createConnection(certdb);
    
    database.connect(function(err) {
        if (err) throw err;
    });

    database.query( 'SELECT nkey FROM x509_certs WHERE nkey=' + serialdecimal, function (error, results, fields) {
      if (error) throw error;
      if (results != '') {
        database.query( 'SELECT * FROM revoked_x509_certs WHERE nkey=' + serialdecimal, function (error, resultsinner, fields) {
          if (error) throw error;
          if (resultsinner != '') {
            logger.debug(JSON.stringify(resultsinner))
            json = JSON.parse(resultsinner[0].nvalue);
            json['Status'] = 'Revoked';

            cb(null, json);
          } else {
            logger.info(`dbresult: certificate valid`)
            json['Status'] = 'Good';
            cb(null, json);
          }
        });
      } else {
        logger.info(`dbresult: unkown certificate`)
        json['Status'] = 'Unkown';
        cb(null, json);
      }
    });
